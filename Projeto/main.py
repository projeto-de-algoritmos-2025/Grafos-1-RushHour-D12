...
SOLVE_BUTTON_COLOR = (200, 100, 100)
...
def draw_grid(surface):
    for x in range(0, GRID_WIDTH * TILE_SIZE + 1, TILE_SIZE): pygame.draw.line(surface, GRAY, (x, 0), (x, GRID_HEIGHT * TILE_SIZE))
    for y in range(0, GRID_HEIGHT * TILE_SIZE + 1, TILE_SIZE): pygame.draw.line(surface, GRAY, (0, y), (GRID_WIDTH * TILE_SIZE, y))
...
def get_block_at(grid_x, grid_y, blocks):
    for block in blocks:
        if block.is_over(grid_x, grid_y): return block
    return None

def is_move_valid(moving_block, new_x, new_y, all_blocks):
    if not (0 <= new_x and new_x + moving_block.width <= GRID_WIDTH and 0 <= new_y and new_y + moving_block.height <= GRID_HEIGHT): return False
    for other_block in all_blocks:
        if other_block is moving_block: continue
        if (new_x < other_block.x + other_block.width and new_x + moving_block.width > other_block.x and new_y < other_block.y + other_block.height and new_y + moving_block.height > other_block.y): return False
    return True
...
def solve_puzzle(initial_blocks, silent=False):
    ...
    if not main_block_props: return None
    ...
    while queue:
        ...
        for i, block_tuple in enumerate(current_state_tuple):
            ...
            possible_moves = []
            if height == 1: possible_moves.extend([(-1, 0), (1, 0)])
            if width == 1: possible_moves.extend([(0, -1), (0, 1)])
    ...
    if not silent: print("Nenhuma solução encontrada.")
    return None

def generate_final_puzzle(screen, font):
    ...
    while True:
        screen.fill(WHITE); screen.blit(loading_text, text_rect); pygame.display.flip(); pygame.event.pump()
        blocks = []
        ...
        for proto in other_blocks_prototypes:
            ...
            for x, y in possible_positions:
                ...
                if new_rect.right > GRID_WIDTH or new_rect.bottom > GRID_HEIGHT: continue
                collides = any((x + i, y + j) in occupied_cells for i in range(new_rect.width) for j in range(new_rect.height))
                if not collides:
                    blocks.append(Block(x, y, proto['width'], proto['height'], proto['color']))
                    for i in range(new_rect.width):
                        for j in range(new_rect.height): occupied_cells.add((x + i, y + j))
                    placed = True
                    break
            if not placed: break
        if len(blocks) != len(other_blocks_prototypes) + 1: continue
...
def main():
    ...
    pygame.display.set_caption("Rush Hour com Solucionador")
    
    big_font = pygame.font.Font(None, 60) 
    button_font = pygame.font.Font(None, 40)

    blocks = generate_final_puzzle(screen, button_font) 
    
    dragging_block, mouse_start_pos, block_start_pos = None, None, None
    game_won = False
    show_solution_message = False

    NEW_GAME_BUTTON_WIDTH = 180 
    SOLVE_BUTTON_WIDTH = 160
    BUTTON_HEIGHT = 60
    BUTTON_GAP = 20 
    
    total_buttons_width = NEW_GAME_BUTTON_WIDTH + SOLVE_BUTTON_WIDTH + BUTTON_GAP
    start_x = (SCREEN_WIDTH - total_buttons_width) / 2
    
    new_game_button = Button(start_x, 620, NEW_GAME_BUTTON_WIDTH, BUTTON_HEIGHT, "Nova Partida", color=BUTTON_COLOR)
    solve_button = Button(start_x + NEW_GAME_BUTTON_WIDTH + BUTTON_GAP, 620, SOLVE_BUTTON_WIDTH, BUTTON_HEIGHT, "Solucionar", color=SOLVE_BUTTON_COLOR)

    solution_path, is_animating, animation_step, last_move_time = [], False, 0, 0
    ...
    while running:
        ...
        if is_animating and not game_won:
            if current_time - last_move_time > 300:
                if animation_step < len(solution_path):
                    move = solution_path[animation_step]
                    block_index, dx, dy = move
                    sorted_blocks = sorted(blocks, key=lambda b: (b.x, b.y, b.width, b.height, b.is_main_block))
                    block_to_move = sorted_blocks[block_index]
                    block_to_move.x += dx
                    block_to_move.y += dy
                    animation_step += 1
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1:
                    if new_game_button.is_clicked(event.pos):
                        blocks = generate_final_puzzle(screen, button_font) # Usar button_font aqui também
                        game_won, dragging_block, solution_path, show_solution_message = False, None, [], False
                        continue
                    
                    if solve_button.is_clicked(event.pos) and not game_won:
                        print("Solver ativado... Buscando a solução.")
                        path = solve_puzzle(blocks)
                        if path:
                            print(f"Solução de {len(path)} movimentos encontrada!")
                            solution_path = path
                            is_animating, animation_step, last_move_time = True, 0, current_time
                        continue

                    if not game_won and not show_solution_message:
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                if event.button == 1: dragging_block = None

            if event.type == pygame.MOUSEMOTION:
                if dragging_block and not game_won and not is_animating: # Adicionei is_animating aqui para garantir
                    mouse_x, mouse_y = event.pos
                    dx, dy = mouse_x - mouse_start_pos[0], mouse_y - mouse_start_pos[1]
                    target_x, target_y = block_start_pos
                    if dragging_block.height == 1: target_x += round(dx / TILE_SIZE)
                    elif dragging_block.width == 1: target_y += round(dy / TILE_SIZE)
        ...
        if game_won:
            text = big_font.render('Você venceu!', True, BLACK, (200, 255, 200))
            text_rect = text.get_rect(center=(SCREEN_WIDTH/2, (GRID_HEIGHT * TILE_SIZE)/2))
            screen.blit(text, text_rect)
        
        if show_solution_message:
            text = big_font.render('Solução encontrada!', True, BLACK, (200, 220, 255))
            text_rect = text.get_rect(center=(SCREEN_WIDTH/2, (GRID_HEIGHT * TILE_SIZE)/2))
            screen.blit(text, text_rect)
